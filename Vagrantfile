...
# Starting Prometheus on the background
prometheus --config.file=/tmp/prometheus.yml


#Give it about 30 seconds to collect data about itself from its own HTTP metrics endpoint.
#http://localhost:9090
#verify that Prometheus is serving metrics about itself by navigating to its own metrics endpoint
...
{
  "experimental": true,
  "metrics-addr": "192.168.54.12:9323"
}
EOF
...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    config.vm.define "vg-prom-04" do |kalicluster|
      ...
      kalicluster.vm.network "private_network", ip: "192.168.54.11"
      ...
      kalicluster.vm.provision "shell", inline: $Prometheus_From_Precompiled_Binary_script #OK
    ...
    config.vm.define "vg-prom-05" do |kalicluster|
      ...
      #bridged network,DHCP enabled,auto IP assignment
      # kalicluster.vm.network "public_network"
      kalicluster.vm.network "private_network", ip: "192.168.54.12"
      # kalicluster.vm.network "forwarded_port", guest: 80, host: 81
      #Disabling the default /vagrant share can be done as follows:
      # kalicluster.vm.synced_folder ".", "/vagrant", disabled: true
      kalicluster.vm.provider "virtualbox" do |vb|
